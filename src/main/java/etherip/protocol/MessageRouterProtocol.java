...
public class MessageRouterProtocol extends ProtocolAdapter
{
    ...
    public void decode(final ByteBuffer buf, final int available, final StringBuilder log) throws Exception {
        ...
        // If the status is different from 0, that means the service was unsuccessfully
        if (this.status != 0) {
            if (this.status == 6) { // Not an error, we need to ask for remaining
            	this.partialTransfert = true;
            } else if (this.status == 5) { // Path destination unknown - the tag wasn't found
                // Don't need to do anything. Just avoid throwing an exception. A CIP data will
                // be generated with invalid data
            }
            else {
